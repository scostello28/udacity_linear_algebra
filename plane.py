...
class Plane(object):
    ...
    def __eq__(self, p):
        if self.normal_vector.zero():
            if not p.normal_vector.zero():
                return False
            else:
                diff = self.constant_term - p.constant_term
                return MyDecimal(diff).is_near_zero()
        elif p.normal_vector.zero():
            return False

        '''Two parallel planes are equal if the vecotr connecting one point on each plane is orthogonal to the planes' normal vectors.'''

        if not self.planes_parallel(p):
            return False
        x0 = self.basepoint
        y0 = p.basepoint
...
if __name__ == '__main__':
    ...
    print('Parallel 1: {}'.format(p1.planes_parallel(p2)))
    ...
    print('Parallel 2: {}'.format(p1.planes_parallel(p2)))
    ...
    print('Parallel 3: {}'.format(p1.planes_parallel(p2)))
